...
IMAGES = {int(os.path.basename(filename)[0]): pygame.transform.smoothscale(pygame.image.load(filename).convert_alpha(), (DROP_LENGTH, DROP_LENGTH)) for filename in glob.glob("images/*.png")}
...
class Pazudora:
    ...
    def main(self):
        ...
        while True:
            ...
            if self.drops_manager.is_time_to_erase():
            ...
            if self.is_moving:
                self.draw_manager.draw_drop(self.moving_drop_type, center=self.moving_drop_pos)
    ...
    def event_handler(self, event):
        ...
        if event.type == KEYDOWN:
            if self.is_moving or self.drops_manager.is_erasing:
                return
            if event.key == K_z:
                self.skills_manager.drops_to_max_combo(self.drops)
            if event.key == K_x:
                self.skills_manager.drops_to_can_all_clear(self.drops)
            if event.key == K_c:
                self.skills_manager.drops_to_exist_5colors(self.drops)
            hanabi_dict = {K_1: 1, K_2: 2, K_3: 3, K_4: 4, K_5: 5, K_6: 6}
            if event.key in hanabi_dict:
                self.skills_manager.drops_to_hanabi(self.drops, hanabi_dict[event.key])
